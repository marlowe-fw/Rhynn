	...
	public:
		...
		static void printRemainingMemoryBlocks() {
			...
			if (memoryBlocks!=0 && memoryBlocks->size() > 0) {
				std::cout << "Num blocks: " << memoryBlocks->size() << std::endl;
				for (MemoryMap::iterator it = memoryBlocks->begin(); it!=memoryBlocks->end(); it++) {
					((*it).second)->print();
				}
			} else {
...
#ifdef _WIN32
inline void __cdecl operator delete(void *p, size_t sz)
#else
...
/*
inline void __cdecl operator delete[] (void* p, const std::nothrow_t& nothrow_constant) {
	operator delete(p);
}
inline void __cdecl operator delete[] (void* p, void* voidptr2) {
	operator delete(p);
}*/
