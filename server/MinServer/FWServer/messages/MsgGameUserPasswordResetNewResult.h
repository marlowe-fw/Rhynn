#ifndef MsgGameUserPasswordResetNewResult_h__
#define MsgGameUserPasswordResetNewResult_h__
...
/**
 * Server informs the client whether or not the password was successfully reset (so).
 */

class MsgGameUserPasswordResetNewResult : public min::MinMessage {
	...
	public:
		...
		MsgGameUserPasswordResetNewResult(bool init = true) {
			msgId = FWSMessageIDs::MSGID_GAME_USER_PASSWORD_RESET_NEW_RESULT;
			_msgMinLength = 6;
		...
		MsgGameUserPasswordResetNewResult(const unsigned char* buf) {
			msgId = FWSMessageIDs::MSGID_GAME_USER_PASSWORD_RESET_NEW_RESULT;
			_msgMinLength = 6;
			_msgIsValid = true;
			valuesFromBytes(buf);
		...
		MsgGameUserPasswordResetNewResult(const unsigned char* buf, unsigned int totalLength) {
			msgId = FWSMessageIDs::MSGID_GAME_USER_PASSWORD_RESET_NEW_RESULT;
			_msgMinLength = 6;
			_msgIsValid = true;
			length = totalLength;
		...
		virtual ~MsgGameUserPasswordResetNewResult() {}
