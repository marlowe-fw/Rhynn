#ifndef MsgGamePlayfieldEnterResult_h__
#define MsgGamePlayfieldEnterResult_h__
...
/**
 * Response from server to notify the client whether or not the character may step on the playfield now.
 */

class MsgGamePlayfieldEnterResult : public min::MinMessage {
	...
	public:
		...
		MsgGamePlayfieldEnterResult(bool init = true) {
			msgId = FWSMessageIDs::MSGID_GAME_PLAYFIELD_ENTER_RESULT;
			_msgMinLength = 6;
		...
		MsgGamePlayfieldEnterResult(const unsigned char* buf) {
			msgId = FWSMessageIDs::MSGID_GAME_PLAYFIELD_ENTER_RESULT;
			_msgMinLength = 6;
			_msgIsValid = true;
			valuesFromBytes(buf);
		...
		MsgGamePlayfieldEnterResult(const unsigned char* buf, unsigned int totalLength) {
			msgId = FWSMessageIDs::MSGID_GAME_PLAYFIELD_ENTER_RESULT;
			_msgMinLength = 6;
			_msgIsValid = true;
			length = totalLength;
		...
		virtual ~MsgGamePlayfieldEnterResult() {}
