#ifndef MsgGameUserPasswordResetCodeResult_h__
#define MsgGameUserPasswordResetCodeResult_h__
...
/**
 * Server informs the client whether or not the password reset-code could be requested (so).
 */

class MsgGameUserPasswordResetCodeResult : public min::MinMessage {
	...
	public:
		...
		MsgGameUserPasswordResetCodeResult(bool init = true) {
			msgId = FWSMessageIDs::MSGID_GAME_USER_PASSWORD_RESET_CODE_RESULT;
			_msgMinLength = 6;
		...
		MsgGameUserPasswordResetCodeResult(const unsigned char* buf) {
			msgId = FWSMessageIDs::MSGID_GAME_USER_PASSWORD_RESET_CODE_RESULT;
			_msgMinLength = 6;
			_msgIsValid = true;
			valuesFromBytes(buf);
		...
		MsgGameUserPasswordResetCodeResult(const unsigned char* buf, unsigned int totalLength) {
			msgId = FWSMessageIDs::MSGID_GAME_USER_PASSWORD_RESET_CODE_RESULT;
			_msgMinLength = 6;
			_msgIsValid = true;
			length = totalLength;
		...
		virtual ~MsgGameUserPasswordResetCodeResult() {}
